...
private:
  ...
  public:
      ...
      void operator =(T num){
          if(num>poly_.Degree()){
              poly_.ResizeCoeffVec(degree_ + 1);
          }
          poly_.PutVal(degree_, num);
          poly_.Shrink();
      }

      operator T() const {// Вызываем константную версию Polynomial::operator[]
...
public:
  ...
  void ResizeCoeffVec(size_t new_size){
      coeffs_.resize(new_size);
  }

  void PutVal(size_t pos, T val){
      coeffs_[pos] = val;
  }
...
void TestCreation() {
  ...
  }
  {
    ...
    from_vector[3] = 0.0;
    ASSERT_EQUAL(from_vector.Degree(), 2);
    from_vector[2] = 0.0;
    ASSERT_EQUAL(from_vector.Degree(), 1);
    from_vector[1] = 0.0;
    ASSERT_EQUAL(from_vector.Degree(), 0);
    from_vector[0] = 0.0;
    ASSERT_EQUAL(from_vector.Degree(), 0);
