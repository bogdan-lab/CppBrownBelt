...
struct Person {
  ...
  bool operator==(const Person& other) const {
    return (this->name==other.name) && (this->weight==other.weight) && (this->height==other.height) && (this->address==other.address);
  }
};
...
struct AddressHasher {
    ...
    size_t operator()(const Address& adr) const {
        size_t c_inp = sh(adr.city);
        size_t s_inp = sh(adr.street);
        size_t b_inp = ih(adr.building);
        size_t x = 2'946'901;
        return c_inp*x*x + s_inp*x + b_inp;
    }
...
struct PersonHasher {
    size_t operator()(const Person& pr) const {
        size_t n_inp = sh(pr.name);
        size_t h_inp = ih(pr.height);
        size_t w_inp = dh(pr.weight);
        size_t a_inp = adrh(pr.address);
        size_t x = 2'946'901;
        return n_inp*x*x*x + h_inp*x*x + w_inp*x + a_inp;
    }
